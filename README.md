...
There is also an older browser-only demo editor:

```text
http://127.0.0.1:5173/admin.html
```

That one only saves in your current browser. The real portal is `/admin/`.

## Important note about the admin form

The admin editor in this prototype is only a demo. It saves edits inside the current browser using `localStorage`.
